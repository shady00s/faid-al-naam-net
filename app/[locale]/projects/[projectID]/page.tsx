...
export default async function Page({
  ...
  const data = await res.json();
  return (
    <section className={" flex   overflow-x-hidden flex-col justify-start "}>
      {data && data.project ? (
        <>
          ...
          <div className="w-screen h-[24rem] relative flex items-center justify-center">
            <Image
              ...
              src={data?.project?.media.find(
                (e: string) => !youtubeRegex.test(e)
              )}
              priority={true}
              fill={true}
          ...
          <div className="w-[80%] m-auto">
            ...
            <div
              className={`flex flex-col ${
                isEnglish ? "justify-end" : "items-end"
              }`}
            >
              <ProjectDetailsTitle
                title={content("steps")}
              ...
              <ProjectDetailsIndecator
                isEnglish={isEnglish}
                numberOfTodos={data?.project?.projectStepsEn.sort(
                  (a: any, b: any) => Number(b.status) - Number(a.status)
                )}
              />
              <div className="h-5"></div>
              {isEnglish
                ? data?.project?.projectStepsEn
                    .sort(
                      (a: any, b: any) => Number(b.status) - Number(a.status)
                    )
            ...
            <div className={"flex flex-col"}>
              ...
              <div className={"w-full  flex justify-center flex-wrap p-4"}>
                {data?.project?.media.map((e: string) =>
                  youtubeRegex.test(e) ? (
                    <iframe
                      key={e}
            ...
            <div className={`flex flex-col `}>
              <ProjectDetailsTitle
                title={content("seeAlso")}
                isEnglish={isEnglish}
              />
              <div
                className={`flex items-center h-full m-auto w-[90%] content-center flex-wrap ${
